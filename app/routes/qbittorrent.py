...
async def get_torrent_tree(
    ...
    if not files_data and content_path:
        ...
        # Use shared path mapping dependency (validates existence)
        try:
            src_path = map_qb_content_path(content_path, validate_exists=True)
        except HTTPException:
            # Path not found - return error response
            logger.warning(f"Filesystem path not accessible: {content_path}")
            return {
                "hash": hash,
                "name": torrent_name,
                "files": [],
